...
private:
    ...
    int m_max_request;          // 请求队列最多允许的，等待处理的请求数量
    std::list<T *> m_workqueue; // 请求队列
    locker m_queuelocker;       // 互斥锁
    sem m_queuestat;            // 信号量判断是否有任务需处理
    bool m_stop;                // 是否结束线程
};

template <typename T>
threadpool<T>::threadpool(int thread_num, int max_requests) : m_thread_num(thread_num), 
                    m_max_request(max_requests), m_stop(false), m_threads(NULL) {
    ...
    for (int i = 0; i < thread_num; ++i)
    {
        ...
        if (pthread_create(m_threads + i, NULL, worker, this) != 0) // work静态函
        {
            delete[] m_threads;
            throw std::exception();
        }
